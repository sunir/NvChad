...
function M.create_sidebar()
  ...
  -- LSP & TreeSitter Integration (language-agnostic)
  vim.keymap.set('n', 'L', '<cmd>lua require("custom.story-explorer").show_lsp_callers()<CR>', opts)
  vim.keymap.set('n', 'S', '<cmd>lua require("custom.story-explorer").show_treesitter_structure()<CR>', opts)
end
...
function M.show_code_structure()
  ...
  if filename == '' then
    ...
    elseif extension == 'py' then
      -- Python file: Use TreeSitter for structure
      local ts_structure = M.get_treesitter_structure()
      ...
      if ts_structure and #ts_structure.children > 0 then
        ...
        table.insert(display_lines, "  (TreeSitter parsing pending - press S)")
      end
    elseif extension == 'lua' then
      -- Lua file: Use TreeSitter for structure
      local ts_structure = M.get_treesitter_structure()

      if ts_structure and #ts_structure.children > 0 then
        table.insert(display_lines, "# " .. basename .. " (Lua)")
        table.insert(display_lines, "")
        table.insert(display_lines, "## Functions")

        for _, child in ipairs(ts_structure.children) do
          table.insert(display_lines, '󰊕 ' .. child.name .. '  [L' .. child.line .. ']')
        end
      ...
      -- Fallback: Try TreeSitter for any language
      local ts_structure = M.get_treesitter_structure()

      if ts_structure and #ts_structure.children > 0 then
        table.insert(display_lines, "# " .. basename .. " (" .. (ts_structure.lang or 'unknown') .. ")")
        table.insert(display_lines, "")

        for _, child in ipairs(ts_structure.children) do
          local icon = child.type == 'class' and '󰠱' or '󰊕'
          table.insert(display_lines, icon .. ' ' .. child.name .. '  [L' .. child.line .. ']')
        end

        table.insert(display_lines, "")
        table.insert(display_lines, "[L] Show LSP callers")
        table.insert(display_lines, "[S] Refresh structure")
      else
        table.insert(display_lines, "📁 " .. basename)
        table.insert(display_lines, "  (No TreeSitter parser for this file type)")
        table.insert(display_lines, "  Press [L] for LSP callers if available")
      end
    end
  end
...
-- LSP-powered callers (language-agnostic)
-- Uses textDocument/references to find all callers of current symbol
function M.get_lsp_callers(callback)
  ...
  if #clients == 0 then
    debug_log('No LSP clients attached')
    callback({})
    return
  end

  debug_log('Requesting LSP references from ' .. #clients .. ' clients')
  ...
  vim.lsp.buf_request_all(0, 'textDocument/references', params, function(results)
    ...
    for client_id, result in pairs(results) do
      if result.result then
        debug_log('Got ' .. #result.result .. ' references from client ' .. client_id)
        for _, ref in ipairs(result.result) do
          local uri = ref.uri or (ref.location and ref.location.uri)
          local range = ref.range or (ref.location and ref.location.range)
          ...
          if uri and range then
            ...
            if lines and lines[line] then
              context = lines[line]:gsub('^%s+', ''):sub(1, 60)
            end
    ...
    debug_log('Found ' .. #callers .. ' LSP callers')
    callback(callers)
  end)
end

-- Show LSP callers in sidebar (async)
function M.show_lsp_callers()
  M.get_lsp_callers(function(callers)
    if #callers == 0 then
      vim.notify('No callers found via LSP', vim.log.levels.INFO)
      return
    end
    ...
    for i, caller in ipairs(callers) do
      local filename = vim.fn.fnamemodify(caller.file, ':t')
      table.insert(lines, string.format('%d. %s:%d', i, filename, caller.line))
      if caller.context ~= '' then
        table.insert(lines, '   ' .. caller.context)
      end
    end
...
function M.get_treesitter_structure()
  ...
  if not tree then
    debug_log('Failed to parse TreeSitter tree')
    return nil
  end

  local root = tree:root()
  local lang = parser:lang()
  debug_log('TreeSitter parsing with language: ' .. lang)
  ...
  if not query_string then
    debug_log('No TreeSitter query for language: ' .. lang)
    return structure
  end

  local ok, query = pcall(vim.treesitter.query.parse, lang, query_string)
  if not ok then
    debug_log('Failed to parse TreeSitter query: ' .. tostring(query))
    return structure
  end
...
-- Show TreeSitter structure in sidebar
function M.show_treesitter_structure()
  local structure = M.get_treesitter_structure()
  ...
  local lines = {
    '# Code Structure (' .. structure.lang .. ')',
    '# ' .. vim.fn.fnamemodify(structure.name, ':t'),
    ''
  }

  for _, child in ipairs(structure.children) do
    local icon = child.type == 'class' and '󰠱' or (child.type == 'method' and '󰊕' or '󰊕')
    table.insert(lines, string.format('%s %s  [L%d]', icon, child.name, child.line))
  end

  if #structure.children == 0 then
    table.insert(lines, '(no structure found)')
  end
